        ...
        double tt_)
{
    ...
    cbeta1 = pow(RMN*tt_/2./PI/HBARC/HBARC,1.5);

    result.f_stability = deiondaa - eion/aa_
        + tt_/aa_*(1.5*log(aa_) + log(aa_*(1.-del_)/2./np_)
                + log(cbeta1) - 2.5);
...
int assign_ocrust_fun_3d(const gsl_vector * x, void *params, gsl_vector * f)
{
    ...
    functs = calc_crust_fun_4d(satdata, sparams, x0, x1, x2, np, 0., tt);
...
struct compo calc_ocrust3d_composition(double nb_, double tt_, double *guess,
...
int assign_icrust_fun_4d(const gsl_vector * x, void *params, gsl_vector * f)
{
    ...
    struct crust_fun_4d functs;
    functs = calc_crust_fun_4d(satdata, sparams, x0, x1, x2, np, x3, tt);
