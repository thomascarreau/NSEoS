...
int calc_zero_temperature_equation_of_state(struct parameters satdata,
  ...
  while (1) {
    comp = calc_ocrust3d_composition(nb, tt, phase, guess_oc, satdata, sparams);
    ...
    pressure =
        calc_crust_ws_cell_pressure(satdata, sparams, comp, nb, tt, phase);
  ...
  while (1) {
    comp = calc_icrust4d_composition(nb, tt, phase, guess_ic, satdata, sparams);
    if (guess_ic[0] != guess_ic[0]) // exit if nan
    {
      if (nb > 0.001 && epsws_core - epsws_ic < 1.e-3) // crust-core transition
      {
    ...
    if (nb > 0.001) {
      ...
      epsws_core =
          calc_core_ws_cell_free_energy_density(satdata, ccomp, nb, tt);
  ...
  while (1) {
    ...
    vs2 = calc_squared_nucleon_sound_velocity(
        satdata, TAYLOR_EXP_ORDER, nb, ccomp.del, tt);
    test_hd = calc_meta_model_nuclear_matter(
        satdata, TAYLOR_EXP_ORDER, nb, ccomp.del, tt);
    ...
    if (*hd_checker == 0 && nb != tqtt->nt && pressure < pressure_sav) {
      fprintf(
          stderr, "HD CHECKER: dP/dnB < 0 (npe core) ; nB = %g /fm^3\n\n", nb);
      *hd_checker = 1;
    }

    if (*hd_checker == 0 && test_hd.jsym < 0) {
      fprintf(
          stderr, "HD CHECKER: Jsym < 0 (npe core) ; nB = %g /fm^3\n\n", nb);
      *hd_checker = 1;
    }
  ...
  while (vs2 > 0. && vs2 < 1.) {
    ...
    if (*hd_checker == 0 && pressure < pressure_sav) {
      fprintf(
          stderr, "HD CHECKER: dP/dnB < 0 (core npeu) ; nB = %g /fm^3\n\n", nb);
      *hd_checker = 1;
    }
    ...
    if (*hd_checker == 0 && test_hd.jsym < 0) {
      fprintf(
          stderr, "HD CHECKER: Jsym < 0 (core npeu) ; nB = %g /fm^3\n\n", nb);
      *hd_checker = 1;
    }
...
void eval_transition_qtt(struct parameters satdata, struct sf_params sparams,
  ...
  while (1) {
    ...
    {
      if (nb > 0.001 && epsws_core - epsws_ic < 1.e-3) {
        fprintf(stderr, "e_core - e_crust = %g\n", epsws_core - epsws_ic);
        break;
      } else {
        return;
    ...
    // calculation of the energy density in the cell in the core
    epsws_core = calc_core_ws_cell_free_energy_density(satdata, ccomp, nb, tt);
