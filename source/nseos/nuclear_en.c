        ...
        double aa_, double ii_, double n0_, double tt_)
{
    ...
    surf_free_en = calc_ls_surface_free_en(satdata, sparams, 
            aa_, ii_, n0_, tt_);
    coul_en = calc_coulomb_en(aa_, ii_, n0_);
        ...
        int d_, struct sf_params sparams, char phase[])
{
    ...
    if (strcmp(phase, "nuclei") == 0) u = (nb_-ng_)/(n0_-ng_);
    else if (strcmp(phase, "bubbles") == 0) u = (n0_-nb_)/(n0_-ng_);
    else
    {
    ...
    sigmac = sigmas
        *sparams.sigma0c/sparams.sigma0*sparams.alpha*(sparams.beta-ypnuc);
