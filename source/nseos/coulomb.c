...
double calc_ugas_energy_density(double nu_)
{
    ...
    egas_energy_density = cpow(MMU,4.)/8./PI2/cpow(HBARC,3.)*((2.*t*t+1.)*t*cpow(t*t+1.,1./2.) - log(t + cpow(t*t+1.,1./2.)));
...
double calc_ugas_chemical_potential(double nu_)
{
    ...
    t = HBARC*cpow(3.0*PI2*nu_,1./3.)/MMU;
    egas_chemical_potential = cpow(MMU,3.)/8./cpow(3.*nu_*PI2,2./3.)/cpow(HBARC,2.)
        *(cpow(t*t+1.,1./2.)*(1.+6.*t*t) + t*t*(2.*t*t+1.)/cpow(t*t+1.,1./2.)
                - 1./cpow(t*t+1.,1./2.));
...
double calc_ugas_pressure(double nu_)
{
    ...
    egas_energy_density = calc_egas_energy_density(nu_);
    egas_chemical_potential = calc_egas_chemical_potential(nu_);
    egas_pressure = nu_*egas_chemical_potential - egas_energy_density;
