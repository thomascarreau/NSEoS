...
double my_integrand(double x, void *params_ptr)
{
    ...
    double my_integrand =  pow(-1,k)*(( 1.+pow(-1,k)*exp(-gamma*x) )
            /( pow(1+exp(-x),gamma) ) - 1.)*pow(x,k);
        ...
        double aa_, double ii_, double n0_)
{
    ...
    alphadenom =  9.*(satdata.lasat0 - ckin*pow(satdata.rhosat0,2./3.)
            *((1.+satdata.barm)/3. - satdata.barm));
    alpha = (alphanum/alphadenom);

    c0num = satdata.lasat0*satdata.ksat0 
        - satdata.ksat0*ckin*pow(satdata.rhosat0,2./3.)*(1.+satdata.barm)
    ...
    c3num = 9.*pow((satdata.lasat0 - ckin*pow(satdata.rhosat0,2./3.)
                *((1.+satdata.barm)/3. - satdata.barm)),2.);
    ...
    clsurf0 = 3.*ckin*pow(satdata.rhosat0,2./3.)
        *(eta_function(0,5./3.)*(1.+satdata.barm) - 3./5.*satdata.barm)
        - 3.*c0*satdata.rhosat0 + 3.*c3*pow(satdata.rhosat0,alpha+1)
        *eta_function(0,alpha+2) ;
    ...
    for( i = 0; i <= imax; i = i + 1 ){
        ...
        sum5 = sum5 + pow(-1,i)*pow(delmsat,i+2)/(i+3)/(i+4)
            *(eta_function(1,i+2) + eta_function(0,i+2)-PI2/3.);
        sum6 = sum6 + pow(-1,i)*pow(delmsat,i)/(i+3)/(i+4)
    ...
    agaus2 = ais*ais + pow((PI/(1.-hnm12.ksym/18./hnm12.jsym)),1./2.)
        *satdata.rhosat0/n0_
    ...
    esurfivcoeff = 3.*pow(PI/(1.-hnm12.ksym/18./hnm12.jsym),1./2.)
        ...
        *(pow(ratioskin,2.)/4. + (ratioskin - pow(ratioskin,2.)/2.)*ii_
                + ((1. - satdata.jsym0/hnm12.jsym)-ratioskin
                    -(1.+satdata.lsym0*hnm12.lsym/hnm12.jsym/satdata.ksat0)/4.
                    *pow(ratioskin,2.))*pow(ii_,2.));
    esurfivsurf = esurfivcoeff*pow(aa_,2./3.) ; 
    esurfivind = esurfivcoeff*2.
        /(1. - hnm12.ksym/18./hnm12.jsym)*pow(aslab/r0,2.) 
...
int be_f (const gsl_vector * x, void *data, gsl_vector * f)
{
    ...
    for (i = 0; i < N; i++) // (wc -l spherical_nuclei.data)
    {
        ...
        double n0 = satdata.rhosat0*(1. - 3.*satdata.lsym0*ii*ii
