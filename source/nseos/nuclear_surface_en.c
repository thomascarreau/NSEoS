...
double calc_ldm_surface_en(struct parameters satdata, double aa_) {
  ...
  // SNM saturation density reference value
...
double my_integrand(double x, void *params_ptr) {
  ...
  double           my_integrand =
      pow(-1, k) *
      ((1. + pow(-1, k) * exp(-gamma * x)) / (pow(1 + exp(-x), gamma)) - 1.) *
      pow(x, k);
...
double eta_function(int a, double b) {
  ...
  gsl_integration_qagiu(&My_function, lower_limit, abs_error, rel_error, 1000,
      work_ptr, &result, &error);

  return result;
}

double calc_etf_ana_surface_en(
    struct parameters satdata, double aa_, double ii_, double n0_) {
  double     ckin;
  ...
  ckin = 3. * HBARC * HBARC / 10. / RMN *
         pow((3. * PI2 / 2.), 2. / 3.); // in MeV.fm^2
  ...
  c0num = satdata.lasat0 * satdata.ksat0 -
          satdata.ksat0 * ckin * pow(satdata.rhosat0, 2. / 3.) *
              (1. + satdata.barm) -
          satdata.lasat0 * ckin * pow(satdata.rhosat0, 2. / 3.) *
              (4. * (1. + satdata.barm) + 21. * satdata.barm) +
  ...
  clsurf = 3. * ckin * pow(n0_, 2. / 3.) *
           ...
           3. * c0 * n0_ +
           3. * c3 * pow(n0_, alpha + 1) * eta_function(0, alpha + 2);

  clcurv =
      6. * ckin * pow(n0_, 2. / 3.) *
          ((eta_function(1, 5. / 3.) - PI2 / 6.) * RMN / rmsat -
              3. / 5. * eta_function(0, 5. / 3.) * delmsat) +
      6. * c3 * pow(n0_, alpha + 1) * (eta_function(1, alpha + 2) - PI2 / 6.);

  clind = 3. * ckin * pow(n0_, 2. / 3.) *
              ((eta_function(2, 5. / 3.) -
                   2. * PI2 / 3. * eta_function(0, 5. / 3.)) *
                      RMN / rmsat -
                  2. / 5. * (3. * eta_function(1, 5. / 3.) - PI2) * delmsat) +
          PI2 * c0 * n0_ +
          3. * c3 * pow(n0_, alpha + 1) *
              (eta_function(2, alpha + 2) -
                  2. * PI2 / 3. * eta_function(0, alpha + 2));

  clsurf0 =
      3. * ckin * pow(satdata.rhosat0, 2. / 3.) *
          (eta_function(0, 5. / 3.) * (1. + satdata.barm) -
              3. / 5. * satdata.barm) -
      3. * c0 * satdata.rhosat0 +
      3. * c3 * pow(satdata.rhosat0, alpha + 1) * eta_function(0, alpha + 2);
  ...
  cnlsurf = HBARC * HBARC / 4. / RMN *
                (1. / 12. - 11. / 36. * delmsat - 1. / 2. * sum1) +
            1. / 2. * cfin * n0_ + 3. * vso * n0_ * n0_ * sum2;
  cnlcurv = HBARC * HBARC / 2. / RMN * (1. / 12. - 1. / 2. * sum3) +
            6. * vso * n0_ * n0_ * sum4;
  cnlind = HBARC * HBARC / 2. / RMN *
               (-PI2 / 12. / 6. + 11. / 36. * (1. + PI2 / 6.) * delmsat) -
           HBARC * HBARC / 4. / RMN * sum5 - (1. + PI2 / 6.) * cfin * n0_ +
           6. * vso * n0_ * n0_ * sum6;
  cnlsurf0 = HBARC * HBARC / 4. / RMN *
                 (1. / 12. - 11. / 36. * satdata.barm - 1. / 2. * sum7) +
             1. / 2. * cfin * satdata.rhosat0 +
             3. * vso * satdata.rhosat0 * satdata.rhosat0 * sum8;
  ...
  drskin =
      drhs * (1. + PI2 / 3. * adif * adif / rhs / rhsp); // n skin thickness
  ratioskin = drskin / adif;

  esurfissurf = (clsurf + cnlsurf / adif / adif) * aratio * pow(aa_, 2. / 3.);
  esurfiscurv =
      (clcurv + cnlcurv / adif / adif) * pow(aratio, 2.) * pow(aa_, 1. / 3.);
  esurfisind = (clind + cnlind / adif / adif) * pow(aratio, 3.);
  esurfis    = esurfissurf + esurfiscurv + esurfisind;

  esurfivcoeff =
      3. * pow(PI / (1. - hnm12.ksym / 18. / hnm12.jsym), 1. / 2.) *
      satdata.rhosat0 / n0_ * aslab / r0 * hnm12.jsym *
      (pow(ratioskin, 2.) / 4. + (ratioskin - pow(ratioskin, 2.) / 2.) * ii_ +
          ((1. - satdata.jsym0 / hnm12.jsym) - ratioskin -
              (1. + satdata.lsym0 * hnm12.lsym / hnm12.jsym / satdata.ksat0) /
                  4. * pow(ratioskin, 2.)) *
              pow(ii_, 2.));
  esurfivsurf = esurfivcoeff * pow(aa_, 2. / 3.);
  esurfivind  = esurfivcoeff * 2. / (1. - hnm12.ksym / 18. / hnm12.jsym) *
                   pow(aslab / r0, 2.) -
               2. * PI2 / 3. * pow(aratio, 2.);
...
int be_f(const gsl_vector *x, void *data, gsl_vector *f) {
  ...
  for (i = 0; i < N; i++) // (wc -l spherical_nuclei.data)
  {
    ...
    struct hnm meta;
    meta =
        calc_meta_model_nuclear_matter(satdata, TAYLOR_EXP_ORDER, n0, ii, 0.);
    double ebulk = meta.enpernuc;
    // surface
    double r0    = pow(4. * PI * n0 / 3., -1. / 3.);
    double ypnuc = (1. - ii) / 2.;
    double sigma = prms.sigma0 * (pow(2., prms.p + 1.) + prms.b) /
                   (pow(ypnuc, -prms.p) + prms.b + pow(1. - ypnuc, -prms.p));
...
double calc_ls_surface_free_en(struct parameters satdata,
    struct sf_params sparams, double aa_, double ii_, double n0_, double tt_) {
  double r0;
  double ypnuc;
  double tc;
  ...
  tc = 87.76 * sqrt(satdata.ksat0 / 375.) *
       pow(0.155 / satdata.rhosat0, 1. / 3.) * ypnuc *
       (1. - ypnuc); // eq. (2.31) in Nuclear Physics A 535 (1991) 331

  if (tt_ > tc)
    h = 0.;
  else
    h = pow(1. - pow(tt_ / tc, 2.), 2.);
  ...
  surf_free_energy = 4. * PI * r0 * r0 * sigma * pow(aa_, 2. / 3.) +
                     8. * PI * r0 * sigma * sparams.sigma0c / sparams.sigma0 *
                         sparams.alpha * (sparams.beta - ypnuc) *
                         pow(aa_, 1. / 3.);
